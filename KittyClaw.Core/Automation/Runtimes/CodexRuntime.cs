...
public sealed class CodexRuntime : IAgentRuntime
{
    private readonly ProcessRunner _processRunner;
    private readonly ILogger<CodexRuntime>? _logger;

    public string Id => AgentRuntimeIds.Codex;

    public CodexRuntime(ProcessRunner processRunner, ILogger<CodexRuntime>? logger = null)
    {
    ...
    public async Task<AgentRunResult> RunAsync(AgentRunRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            var tempFile = Path.Combine(Path.GetTempPath(), $"kittyclaw-codex-{Guid.NewGuid():N}.txt");
