...
public sealed class AntigravityRuntime : IAgentRuntime
{
    private readonly ProcessRunner _processRunner;
    private readonly ILogger<AntigravityRuntime>? _logger;

    public string Id => AgentRuntimeIds.Antigravity;

    public AntigravityRuntime(ProcessRunner processRunner, ILogger<AntigravityRuntime>? logger = null)
    {
    ...
    public async Task<AgentRunResult> RunAsync(AgentRunRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            var tempFile = Path.Combine(Path.GetTempPath(), $"kittyclaw-antigravity-{Guid.NewGuid():N}.txt");
