...
public sealed class GitHubCopilotRuntime : IAgentRuntime
{
    private readonly ProcessRunner _processRunner;
    private readonly ILogger<GitHubCopilotRuntime>? _logger;

    public string Id => AgentRuntimeIds.GitHubCopilot;

    public GitHubCopilotRuntime(ProcessRunner processRunner, ILogger<GitHubCopilotRuntime>? logger = null)
    {
    ...
    public async Task<AgentRunResult> RunAsync(AgentRunRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            var tempFile = Path.Combine(Path.GetTempPath(), $"kittyclaw-copilot-{Guid.NewGuid():N}.txt");
