...
public sealed class OpenCodeRuntime : IAgentRuntime
{
    private readonly ProcessRunner _processRunner;
    private readonly ILogger<OpenCodeRuntime>? _logger;

    public string Id => AgentRuntimeIds.OpenCode;

    public OpenCodeRuntime(ProcessRunner processRunner, ILogger<OpenCodeRuntime>? logger = null)
    {
    ...
    public async Task<AgentRunResult> RunAsync(AgentRunRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            var tempFile = Path.Combine(Path.GetTempPath(), $"kittyclaw-opencode-{Guid.NewGuid():N}.txt");
